...
name: options
description: >
  Use when the question is what could actually be done: generating courses of
  action for a defined problem, repairing a weak or narrow option set, working
  out what the relevant actor can genuinely change, or turning a vague ambition
  into concrete policy choices. Also use when the options on the table are a
...
Canonical method:
  ...
  - alternatives as hypotheses, iteration with the problem frame, policy
    variables and feasible manipulations, packaging into strategies, strategic
    thrust, business-as-usual, straw alternatives, false differentiation and
    false singularity, narrowing, system design and transition. Provenance:
    Bardach Step Three and Appendix B; Peter J. May, Hints for Crafting
    Alternative Policies (1981).

Distilled shared behaviour carried here:
  reference/methods/shared/uncertainty-and-learning-guidance.md - only the
    decision about whether "learn first" belongs in the option set, and the
    requirement that a learning option names its uncertainty and decision rule.
    Designing the learning move belongs to `evidence`.
  reference/methods/shared/risk-opportunity-appraisal-guidance.md - only
    package and pathway construction, and lock-in as a reason to consider a
    staged option. The appraisal itself belongs to `criteria` and `outcomes`.
