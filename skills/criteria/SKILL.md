...
name: criteria
description: >
  Use when the question is what counts as better: deciding what the options
  should be judged against, what the primary objective actually is, which
  requirements are hard constraints rather than tradable values, and how
  distribution, rights or process values should be made explicit. Also use when
  options are hard to compare because the standards were never stated, when
  "equity" or "value for money" or "customer experience" is being used as a
...
Canonical method:
  ...
  - evaluative versus practical criteria, primary objectives, efficiency,
    equity, rights, process values, legality, political acceptability,
    robustness, improvability, roles, direction, proxies, weighting and
    aggregation. Provenance: Bardach Step Four.

Distilled shared behaviour carried here:
  reference/methods/shared/risk-opportunity-appraisal-guidance.md - only the
    appraisal gate (marginality, heterogeneity, quantifiable versus fundamental
    uncertainty), the direction-of-change question, and the warning against
    forcing non-commensurable values into one metric. The full appraisal method
    stays in the file.
  reference/methods/shared/strategic-triangle-guidance.md - only the separation
    of public value from political support, and practical feasibility as a
